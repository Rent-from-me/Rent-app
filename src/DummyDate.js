export const init = {
  users: {
    profile: {
      name: "user",
      password: "123456",
      img: "https://bit.ly/3ofw4lB",
      location: "Hargiesa",
      dateJoined: "mar-17-20",
    },
    Rentuser: [
      {
        id: 1,
        img: "../.././asset/camer1.jpg",
        title: "camera one",
        price: "50",
        review: 1,
        date: "mar-7-21",
        reviewer: 26,
        desc: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book",
      },
    ],
    msgUser: {
      msg: ["msg form hoster"],
    },
  },
  rentItem: [
    {
      id: 1,
      img: {
        main: "https://bit.ly/3HkTSfg",
        sub1: "https://bit.ly/3HkU9Pk",
        sub2: "https://bit.ly/3ueqYJV",
        sub3: "https://bit.ly/3rjqxfN",
        sub4: "https://bit.ly/3GgPBbl",
      },
      title: "Kodak PIXPRO FZ53 16.15MP",
      location: "Hargiesa",
      price: "50",
      review: 1,
      date: "mar-7-21",
      reviewer: 26,
      desc: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book",
      comments: [
        {
          name: "hassan ahmed",
          date: "jan-27-22",
          img: "https://bit.ly/3AQEKUD",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "jacfar muusa",
          date: "jan-27-22",
          img: "https://bit.ly/3ogbeCw",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "saynab jaamac",
          date: "jan-27-22",
          img: "https://bit.ly/3rhfPWQ",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "farhan muxumed",
          date: "jan-27-22",
          img: "https://bit.ly/3L3wXra",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
      ],
    },
    {
      id: 2,
      img: {
        main: "https://bit.ly/3HkIxMl",
        sub1: "https://bit.ly/3HkU9Pk",
        sub2: "https://bit.ly/3ueqYJV",
        sub3: "https://bit.ly/3rjqxfN",
        sub4: "https://bit.ly/3GgPBbl",
      },
      title: "Kodak PIXPRO FZ53 16.15MP",
      location: "Hargiesa",
      price: "50",
      review: 1,
      date: "mar-7-21",
      reviewer: 26,
      desc: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book",
      comments: [
        {
          name: "hassan ahmed",
          date: "jan-27-22",
          img: "https://bit.ly/3AQEKUD",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "jacfar muusa",
          date: "jan-27-22",
          img: "https://bit.ly/3ogbeCw",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "saynab jaamac",
          date: "jan-27-22",
          img: "https://bit.ly/3rhfPWQ",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "farhan muxumed",
          date: "jan-27-22",
          img: "https://bit.ly/3L3wXra",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
      ],
    },
    {
      id: 3,
      img: {
        main: "https://bit.ly/3L4Cxt6",
        sub1: "https://bit.ly/3HkU9Pk",
        sub2: "https://bit.ly/3ueqYJV",
        sub3: "https://bit.ly/3rjqxfN",
        sub4: "https://bit.ly/3GgPBbl",
      },
      title: "Kodak PIXPRO FZ53 16.15MP",
      location: "Hargiesa",
      price: "50",
      review: 1,
      date: "mar-7-21",
      reviewer: 26,
      desc: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book",
      comments: [
        {
          name: "hassan ahmed",
          date: "jan-27-22",
          img: "https://bit.ly/3AQEKUD",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "jacfar muusa",
          date: "jan-27-22",
          img: "https://bit.ly/3ogbeCw",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "saynab jaamac",
          date: "jan-27-22",
          img: "https://bit.ly/3rhfPWQ",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "farhan muxumed",
          date: "jan-27-22",
          img: "https://bit.ly/3L3wXra",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
      ],
    },
    {
      id: 4,
      img: {
        main: "https://bit.ly/3HrQ4ck",
        sub1: "https://bit.ly/3HkU9Pk",
        sub2: "https://bit.ly/3ueqYJV",
        sub3: "https://bit.ly/3rjqxfN",
        sub4: "https://bit.ly/3GgPBbl",
      },
      title: "Kodak PIXPRO FZ53 16.15MP",
      location: "Hargiesa",
      price: "50",
      review: 1,
      date: "mar-7-21",
      reviewer: 26,
      desc: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book",
      comments: [
        {
          name: "hassan ahmed",
          date: "jan-27-22",
          img: "https://bit.ly/3AQEKUD",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "jacfar muusa",
          date: "jan-27-22",
          img: "https://bit.ly/3ogbeCw",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "saynab jaamac",
          date: "jan-27-22",
          img: "https://bit.ly/3rhfPWQ",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "farhan muxumed",
          date: "jan-27-22",
          img: "https://bit.ly/3L3wXra",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
      ],
    },
    {
      id: 5,
      img: {
        main: "https://bit.ly/3J0Uh7b",
        sub1: "https://bit.ly/3HkU9Pk",
        sub2: "https://bit.ly/3ueqYJV",
        sub3: "https://bit.ly/3rjqxfN",
        sub4: "https://bit.ly/3GgPBbl",
      },
      title: "Kodak PIXPRO FZ53 16.15MP",
      location: "Hargiesa",
      price: "50",
      review: 1,
      date: "mar-7-21",
      reviewer: 26,
      desc: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book",
      comments: [
        {
          name: "hassan ahmed",
          date: "jan-27-22",
          img: "https://bit.ly/3AQEKUD",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "jacfar muusa",
          date: "jan-27-22",
          img: "https://bit.ly/3ogbeCw",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "saynab jaamac",
          date: "jan-27-22",
          img: "https://bit.ly/3rhfPWQ",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
        {
          name: "farhan muxumed",
          date: "jan-27-22",
          img: "https://bit.ly/3L3wXra",
          comment:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley",
        },
      ],
    },
  ],

  host: {
    msg: ["form the user"],
    rentitems: [
      {
        id: 1,
        img: "https://bit.ly/3ofZ8Ju",
        title: "camera one",
        price: "50",
        review: 1,
        date: "mar-7-21",
        reviewer: 26,
        desc: "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book",
      },
    ],
  },
};
